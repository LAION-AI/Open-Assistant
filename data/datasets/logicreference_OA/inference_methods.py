...
def rename_clauses(exp, suffix):
    ...
    if isinstance(exp, list):
        ...
        return lib.InferenceRule(
            rename_clauses(exp.premises, suffix),
...
def unify_clauses(p1, p2, bindings=None, allow_p2_bindings=True):
    ...
    if len(p1) == 1 and len(p2) == 1:
        if is_base_proposition(p1[0]) and is_base_proposition(p2[0]):
            if (p1[0] not in bindings) or p2 == bindings[p1[0]] or p2 == [bindings[p1[0]]]:
    ...
    if len(p1) == 1 and is_base_proposition(p1[0]) and len(p2) == 2 and is_functor(p2[0]) and is_constant(p2[1]):
    ...
    if len(p1) == 2 and is_functor(p1[0]) and len(p2) == 2 and is_functor(p2[0]):
        if (is_constant(p1[1]) and is_constant(p2[1])) or (is_variable(p1[1]) and is_variable(p2[1])):
            if (p1[0] not in bindings) or p2[0] == bindings[p1[0]]:
                bindings1 = bindings.copy()
                bindings1[p1[0]] = p2[0]
    ...
    # match "->"", "<->", "or", "and", "~" to P(c):
    if (p1[0] in ["->", "<->", "or", "and", "~"]) and is_functor(p2[0]) and is_constant(p2[1]) and allow_p2_bindings:
        ground_propositions = []
        ground = True
        for p in p1[1:]:
    ...
    # match "exists"/"forall":
    if len(p1) == 3 and len(p2) == 3 and p1[0] == p2[0] and (p1[0] in ["exists", "forall"]):
        # match variable:
        if (p1[1] not in bindings) or p2[1] == bindings[p1[1]]:
            bindings1 = bindings.copy()
            bindings1[p1[1]] = p2[1]
...
def apply_bindings(bindings, exp):
    ...
    if isinstance(exp, list):
            ...
            if len(exp) == 1 and is_base_proposition(exp[0]) and exp[0] in bindings:
                ...
                if isinstance(bindings[exp[0]], str):
                    ...
                    for i in range(len(new_exp)):
                        if isinstance(new_exp[i], list) and len(new_exp[i]) == 1 and is_functor(new_exp[i][0]):
        ...
        if exp in bindings:
            ...
            if (
                is_base_proposition(exp)
                and isinstance(new_exp, list)
                and len(new_exp) == 1
                and is_base_proposition(new_exp[0])
            ):
                return new_exp[0]
            return new_exp
    ...
    for p1 in premises:
        for p2 in premises:
            if p1 == ["~", p2]:
                # Contradiction! (2)
                return [p1, p2]
            if isinstance(p1, list) and isinstance(p2, list) and p1[0] == "forall":
                p1_sign = True
                p1_internal = p1[2]
                ...
                if p1_internal[0] == p2_internal[0] and p1_sign != p2_sign:
                    # Contradiction! (3)
                    return [p1, p2]
...
def premises_match_rule(premises, rule, next_idx=0, bindings=None, prevent_equal_bindings=True):
    ...
    if next_idx < len(rule.premises):
        ...
        for premise in premises:
            bindings2 = unify_clauses(rule_premise, premise, bindings, allow_p2_bindings=False)
            if (bindings2 is not None) and (
                (not prevent_equal_bindings) or (not bindings_contain_equal_binding(bindings2))
            ):
                results2 = premises_match_rule(premises, rule, next_idx + 1, bindings2, prevent_equal_bindings)
                results += results2
        return results
    else:
        return [bindings]
...
def one_step_inferences_from_premises(premises, rules_to_ignore=None):
