...
def render_language_clause(clause, propositions, nl_propositions, form=0):
    ...
    if clause[0] == "~":
        return render_language_clause(clause[1], propositions, nl_propositions, form=(form + 3) % 6)

    elif clause[0] == "forall":
        argument1 = render_language_clause(clause[2], propositions, nl_propositions, form=2)
        ...
        if clause[0] == "->":
            argument1 = render_language_clause(clause[1], propositions, nl_propositions, form=1)
            argument2 = render_language_clause(clause[2], propositions, nl_propositions, form=0)
        else:
            argument1 = render_language_clause(clause[1], propositions, nl_propositions, form=form % 3)
            argument2 = render_language_clause(clause[2], propositions, nl_propositions, form=form % 3)
        if form < 3:
            # positive:
            if clause[0] == "->":
...
def generate_nl_proposition(functor, arguments, bindings):
    ...
    if functor and (functor in bindings):
        if isinstance(bindings[functor], str):
            ...
            if len(bindings[functor]) == 3:
                ...
                raise ValueError(
                    "proposition bound to a value not found in either " "actions nor impsersonal_candidates!"
                )
    ...
    if len(arguments) == 1:
        ...
        raise ValueError("Propositions with more than 1 argument not " + f"supported!: {functor} + {arguments}")
    ...
    if proposition_type == "impersonal":
        ...
        return (
            ...
            f"{subject} were {predicate}",
            f"{subject} is {predicate}",
            f"{subject} is not {predicate}",
            f"{subject} were not {predicate}",
            f"{subject} is not {predicate}",
        )
        ...
        if functor:
            bindings[functor] = predicate
        return (
            f"{subject} is {predicate[2]}",
            f"{subject} {predicate[1]}",
...
def precompute_rules():
    ...
    lib.PROPOSITIONAL_INFERENCE_RULES = [
        ...
        lib.InferenceRule([["->", ["p"], ["q"]], ["q"]], [], [], [["p"]], ["p", "q"], "incorrect modus tollens"),
        lib.InferenceRule(
            [["->", ["p"], ["q"]], ["->", ["q"], ["r"]]],
            [["->", ["p"], ["r"]]],
            [["~", ["->", ["p"], ["r"]]]],
        ...
        lib.InferenceRule([["or", ["p"], ["~", ["q"]]]], [["->", ["q"], ["p"]]], [], [], ["p", "q"], "rewriting"),
        lib.InferenceRule(
            [["->", ["p"], ["r"]], ["->", ["q"], ["r"]], ["or", ["p"], ["q"]]],
            [["r"]],
            [["~", "r"]],
        ...
        lib.InferenceRule(
            [["<->", ["p"], ["q"]], ["~", ["p"]]], [["~", ["q"]]], [["q"]], [], ["p", "q"], "biconditional elimination"
        ),
        lib.InferenceRule(
            [["<->", ["p"], ["q"]], ["~", ["q"]]], [["~", ["p"]]], [["p"]], [], ["p", "q"], "biconditional elimination"
        ),
    ]
    ...
    lib.QUANTIFIED_INFERENCE_RULES = [
        lib.InferenceRule(
            ...
            [["~", ["P", "a"]]],  # contradictions
            [["Q", "a"], ["~", ["Q", "a"]]],  # irrelevant
            [["P", "var x"], ["P", "a"], ["Q", "a"]],  # list of propositions
    ...
    lib.ALL_INFERENCE_RULES = lib.PROPOSITIONAL_INFERENCE_RULES + lib.QUANTIFIED_INFERENCE_RULES
