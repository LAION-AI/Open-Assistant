...
import datasets
import torch
from model_training.custom_datasets.ranking_collator import RankingDataCollatorWithLabel
...
class RMTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_logits=False):
        ...
        loss = self.loss_fct(logits, cu_lens, labels)
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            ...
            loss, logits = self.compute_loss(model, (batch, cu_lens, labels), return_logits=True)
...
def main():
    ...
    train_collate_fn = RankingDataCollatorWithLabel(
        tokenizer,
        max_length=training_conf.max_length,
        pad_to_multiple_of=16,
        max_replies=training_conf.max_replies,
    )
    eval_collate_fn = RankingDataCollatorWithLabel(
        tokenizer,
        max_length=training_conf.max_length,
        pad_to_multiple_of=16,
        max_replies=training_conf.max_replies,
