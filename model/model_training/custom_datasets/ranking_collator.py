...
from .formatting import ATTRIBUTE_KEYS, format_pairs, format_reply


@dataclass
class RankingDataCollatorWithLabel:
    """
    ...
    def process_one(
        ...
        if isinstance(example, DatasetEntry):
            # ugh?
            prefix, replies, labels = example.get_formatted(mode=Mode.rm, eos_token=eos)
        else:
            messages, replies, labels = example
            ...
            if self.max_replies:
                ...
                if len(replies) > self.max_replies:
                    ...
                    labels = labels[: self.max_replies]
        ...
        label_arr = torch.Tensor([[label[key] for key in ATTRIBUTE_KEYS] for label in labels])
    ...
    def __call__(
        ...
        return (batch, cu_lens, torch.cat(labels, dim=0))
