...
def main():
    signal.signal(signalnum=signal.SIGINT, handler=terminate_worker)
    ...
    while True:
        try:
            if not model_config.is_lorem:
                utils.wait_for_inference_server(http=inference_http)

            if settings.perform_oom_test:
                work.perform_oom_test(tokenizer=tokenizer)
                sys.exit(__status=0)
            ...
            with closing(
                thing=websocket.create_connection(
                    url=f"{settings.backend_url}/workers/work",
                    header={
                ...
                utils.send_response(ws=ws, response=worker_info)
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=worker_config.max_parallel_requests) as executor:
                    ...
                    while True:
                        if ftrs:
                            done, not_done = concurrent.futures.wait(fs=ftrs, timeout=1)
                            ftrs = list(not_done)
                            for ftr in done:
                                ftr.result()
                        message = ws.recv()
                        ...
                        match worker_request.request_type:
                            ...
                            case "ping":
                                utils.send_response(
                                    ws=ws,
                                    response=inference.PongResponse(
