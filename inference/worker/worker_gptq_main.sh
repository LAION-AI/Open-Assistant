...
# TODO: so far only local models supported
echo "Not downloading model $MODEL_ID for the time being, must be available locally"
...
if [ -z "$CUDA_VISIBLE_DEVICES" ]; then
    ...
    python3 -m uvicorn --host 0.0.0.0 --port $worker_port local_gptq_server:app &
    ...
    for i in "${!devices[@]}"; do
        ...
        CUDA_VISIBLE_DEVICES=$device python3 -m uvicorn --host 0.0.0.0 --port $worker_port local_gptq_server:app &
