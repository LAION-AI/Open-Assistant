...
async def callback_discord(
    code: str,
    session: deps.AsyncSession = Depends(deps.create_session),
):
    redirect_uri = f"{settings.api_root}/auth/callback/discord"
    ...
    user: models.DbUser = await query_user_by_provider_id(session, discord_id=discord_id)
...
async def query_user_by_provider_id(session: deps.AsyncSession, discord_id: str | None = None) -> models.DbUser | None:
    ...
    user: models.DbUser = (await session.exec(user_qry)).first()
    return user


@app.get("/auth/login/debug")
async def login_debug(username: str, session: deps.AsyncSession = Depends(deps.create_session)):
    ...
    user: models.DbUser = (
        await session.exec(sqlmodel.select(models.DbUser).where(models.DbUser.id == username))
    ).one_or_none()
