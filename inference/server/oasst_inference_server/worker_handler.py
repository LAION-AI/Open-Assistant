...
from oasst_inference_server import deps, models, queueing, worker_handler
...
async def find_compliance_work_request_message(
    session: deps.AsyncSession, worker_config: inference.WorkerConfig, worker_id: str
...
async def should_do_compliance_check(session: deps.AsyncSession, worker_id: str) -> bool:
...
async def run_compliance_check(websocket: fastapi.WebSocket, worker_id: str, worker_config: inference.WorkerConfig):
    async with deps.manual_create_session() as session:
        try:
            worker = await get_worker(worker_id, session, with_for_update=True)
            if worker.in_compliance_check:
